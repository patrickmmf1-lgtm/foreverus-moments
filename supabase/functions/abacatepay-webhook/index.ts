...
// Rate limiting configuration for webhooks (stricter to prevent replay attacks)
...
serve(async (req) => {
  ...
  try {
    ...
    if (!rateLimit.allowed) {
      console.warn(`Rate limit exceeded for webhook from IP: ${clientIP.substring(0, 8)}...`);
      return new Response(
